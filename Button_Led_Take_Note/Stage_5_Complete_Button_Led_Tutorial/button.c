...
void button_fsm_process(Button_Context *btn, unsigned int current_tick) {
    ...
    if (current_tick - btn->last_update_tick >= 10) {
        ...
        if (btn->pin_state == 0) { 
            ...
            if (btn->press_duration == 500) { // 1000 nhịp * 10ms = 10s
            ...
            if (btn->press_duration > 0) { 
								// [ĐÃ SỬA BUGS]: Giới hạn trên phải bằng đúng thời gian Hold (500)
                if (btn->press_duration > 3 && btn->press_duration < 500) {
                    btn->click_count++;
                }
                btn->press_duration = 0; // Nhả rồi thì reset thời gian đè
            }
